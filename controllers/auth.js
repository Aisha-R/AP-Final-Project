...
exports.delete_gp = async (req, res) => {
    try {

        const gp = await Gp.query().select('gpAddressId').where('id', req.params.gpId).limit(1);

        const doctors = await Doctor.query().select('id').where('gpId', req.params.gpId);

        for ( entry in doctors ) {
            await Doctor.query().deleteById(doctors[entry].id);
        }
